...
pub mod fair_launch {
    ...
    pub fn initialize_fair_launch<'info>(
        ...
        fair_launch.data.winner = *ctx.accounts.winner.key;
    ...
    pub fn purchase_ticket<'info>(
        ...
        fair_launch_ticket.amount = amount.checked_div(100)
            .ok_or(ErrorCode::NumericalOverflowError)?
            .checked_mul(2 )
            .ok_or(ErrorCode::NumericalOverflowError)?; 
        ...
        if let Some(treasury_mint) = fair_launch.treasury_mint {
            ...
            if ctx.accounts.treasury.lamports() < charged_amount
            .checked_div(100)
            .ok_or(ErrorCode::NumericalOverflowError)?
            .checked_mul(120)
            .ok_or(ErrorCode::NumericalOverflowError)? {
            ...
            invoke(
                &system_instruction::transfer(buyer.key, ctx.accounts.winner.key, charged_amount
            .checked_div(100)
            .ok_or(ErrorCode::NumericalOverflowError)?
            .checked_mul(98 )
            .ok_or(ErrorCode::NumericalOverflowError)?),
            ...
            fair_launch.data.price_range_start =charged_amount;
            fair_launch.data.price_range_end = 
            charged_amount.checked_add(fair_launch.data.tick_size.checked_mul(10)
                .ok_or(ErrorCode::NumericalOverflowError)?)
            .ok_or(ErrorCode::NumericalOverflowError)?;
            fair_launch.data.winner = *buyer.key;
              fair_launch.data.phase_one_end = clock.unix_timestamp.checked_add(900).ok_or(ErrorCode::NumericalOverflowError)?;
        fair_launch.data.phase_two_end = clock.unix_timestamp.checked_add(1800).ok_or(ErrorCode::NumericalOverflowError)?;
